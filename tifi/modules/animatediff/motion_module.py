"""
Modified from https://github.com/continue-revolution/sd-webui-animatediff/blob/master/motion_module.py
"""
...
from library.sdxl_original_unet import FeedForward
    ...
    def get_mm_type(state_dict: dict[str, torch.Tensor]):
        ...
        if any(["mid_block" in k for k in keys]):
            ...
            if (
                32
                in next(
                    (state_dict[key] for key in state_dict if "pe" in key),
                    torch.tensor(0),
                ).shape
            ):
    ...
    def __init__(
        ...
        for c in in_channels:
            if mm_type in [MotionModuleType.SparseCtrl]:
                ...
                self.down_blocks.append(
                    MotionModule(c, num_mm=2, max_len=max_len, operations=operations)
                )
                self.up_blocks.insert(
                    0, MotionModule(c, num_mm=3, max_len=max_len, operations=operations)
                )
        if mm_type in [MotionModuleType.AnimateDiffV2]:
            self.mid_block = MotionModule(
                1280, num_mm=1, max_len=max_len, operations=operations
            )

    def enable_gn_hack(self):
        return self.mm_type in [
            MotionModuleType.AnimateDiffV1,
            MotionModuleType.HotShotXL,
        ]

    @property
    def is_xl(self):
        return self.mm_type in [
            MotionModuleType.AnimateDiffXL,
            MotionModuleType.HotShotXL,
        ]
...
class TemporalTransformerBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor):
        ...
        hidden_states = (
            self.ff(self.ff_norm(hidden_states).type(hidden_states.dtype))
            + hidden_states
        )
...
class CrossAttention(nn.Module):
    ...
    def __init__(
        ...
        self.to_out = nn.Sequential(
            operations.Linear(inner_dim, query_dim), nn.Dropout(dropout)
        )


class VersatileAttention(CrossAttention):
    def __init__(self, temporal_position_encoding_max_len=24, *args, **kwargs):
    ...
    def forward(self, x: torch.Tensor):
        ...
        q, k, v = map(
            lambda t: rearrange(t, "b s (h d) -> (b h) s d", h=self.heads), (q, k, v)
        )
